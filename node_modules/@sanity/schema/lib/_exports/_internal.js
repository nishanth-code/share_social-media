...
function groupProblems(types) {
  return lodash.flatten(types.map(type => getTypeProblems(type))).filter(type => type.problems.length > 0);
}
function createTypeWithMembersProblemsAccessor(memberPropertyName) {
  let getMembers = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : type => lodash.get(type, memberPropertyName);
...
function traverseSchema() {
  ...
  const duplicateNames = lodash.uniq(lodash.flatten(getDupes(typeNames)));
...
var reference = (typeDef, visitorContext) => {
  const isValidTo = Array.isArray(typeDef.to) || lodash.isPlainObject(typeDef.to);
  const normalizedTo = normalizeToProp$1(typeDef);
  const problems = lodash.flatten([isValidTo ? getDupes(normalizedTo, t => "".concat(t.name, ";").concat(t.type)).map(dupes => error("Found ".concat(dupes.length, " members with same type, but not unique names \"").concat(dupes[0].type, "\" in reference. This makes it impossible to tell their values apart and you should consider naming them"), HELP_IDS.REFERENCE_TO_INVALID)) : error('The reference type is missing or having an invalid value for the required "to" property. It should be an array of accepted types.', HELP_IDS.REFERENCE_TO_INVALID)]);
...
var array = (typeDef, visitorContext) => {
  ...
  const problems = lodash.flatten([ofIsArray ? getDupes(typeDef.of, t => "".concat(t.name, ";").concat(t.type)).map(dupes => error("Found ".concat(dupes.length, " members with same type, but not unique names \"").concat(dupes[0].type, "\" in array. This makes it impossible to tell their values apart and you should consider naming them"), HELP_IDS.ARRAY_OF_NOT_UNIQUE)) : error('The array type is missing or having an invalid value for the required "of" property', HELP_IDS.ARRAY_OF_INVALID)]);
  ...
  const [primitiveTypes, objectTypes] = lodash.partition(of, ofType => {
    var _a2;
...
const allowedKeys = ["lists", "marks", "name", "of", "options", "styles", "title", "type", "validation"];
...
function validateBlockType(typeDef, visitorContext) {
  ...
  return _objectSpread(_objectSpread({}, lodash.omit(typeDef, disallowedKeys)), {}, {
    marks,
    styles,
    of: members,
    _problems: problems
...
var crossDatasetReference = (typeDef, visitorContext) => {
  ...
  const problems = lodash.flatten([isValidTo ? getDupes(normalizedTo, t => "".concat(t.name, ";").concat(t.type)).map(dupes => error("Found ".concat(dupes.length, " members with same type, but not unique names \"").concat(dupes[0].type, "\" in reference. This makes it impossible to tell their values apart and you should consider naming them"), HELP_IDS.CROSS_DATASET_REFERENCE_INVALID)) : error('The cross dataset reference type is missing or having an invalid value for the required "to" property. It should be an array of accepted types.', HELP_IDS.CROSS_DATASET_REFERENCE_INVALID)]);
  if (isValidTo && normalizedTo.length === 0) {
...
const validateActions = (typeName, actions) => {
  ...
  const invalid = lodash.difference(actions, VALID_ACTIONS);
  if (invalid.length > 0) {
