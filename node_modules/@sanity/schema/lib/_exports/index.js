...
function parsePreview(preview) {
  ...
  if (Array.isArray(select)) {
    return _objectSpread(_objectSpread({}, lodash.pick(preview, ["prepare", "component"])), {}, {
...
function getCachedStringFieldPaths(type, maxDepth) {
  if (!type[stringFieldsSymbol]) {
    type[stringFieldsSymbol] = lodash.uniqBy([...BASE_WEIGHTS, ...deriveFromPreview(type), ...getStringFieldPaths(type, maxDepth).map(path => ({
      weight: 1,
      path
...
function getCachedBaseFieldPaths(type) {
  if (!type[stringFieldsSymbol]) {
    type[stringFieldsSymbol] = lodash.uniqBy([...BASE_WEIGHTS, ...deriveFromPreview(type)], spec => spec.path.join("."));
  }
  return type[stringFieldsSymbol];
}
      ...
      return {
        ...
        extend: extensionDef => {
          ...
          const current = Object.assign({}, parent, lodash.pick(extensionDef, OVERRIDABLE_FIELDS$5), {
            title: extensionDef.title || subTypeDef.title || (subTypeDef.name ? lodash.startCase(subTypeDef.name) : ""),
            type: parent
          });
...
function createFieldsGroups(typeDef, fields) {
  ...
  if (defaultGroups.length > 1) {
    throw new Error("You currently have ".concat(defaultGroups.length, " default field groups defined for type '").concat(typeDef.name ? lodash.startCase(typeDef.name) : (_a = typeDef.title) != null ? _a : "", "', but only 1 is supported"));
  }
...
function buildTitle(type) {
  ...
  return "Cross dataset reference to ".concat(humanize(arrify__default["default"](type.to).map(toType => toType.title || lodash.capitalize(toType.type)), "or").toLowerCase());
}
