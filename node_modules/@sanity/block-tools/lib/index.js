...
lodash.uniq(Object.values(ELEMENT_MAP).filter(tag => "style" in tag).map(tag => tag.style));
  ...
  blocks.forEach(block => {
    ...
    block.children.forEach((child, index) => {
      ...
      if (prevChild && lodash.isEqual(prevChild.marks, child.marks) && isWhiteSpaceChar(child.text)) {
        prevChild.text += " ";
        block.children.splice(index, 1);
      } else if (nextChild && lodash.isEqual(nextChild.marks, child.marks) && isWhiteSpaceChar(child.text)) {
...
function normalizeBlock(node) {
  ...
  block.children = block.children.reduce((acc, child) => {
    ...
    if (previousChild && isSpan(child) && isSpan(previousChild) && lodash.isEqual(previousChild.marks, child.marks)) {
