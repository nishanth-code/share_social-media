...
function validateValue(value, portableTextFeatures, keyGenerator) {
  ...
  if (value.some((blk, index) => {
    ...
    if (blk._type === portableTextFeatures.types.block.name) {
      ...
      const allUsedMarks = lodash.uniq(lodash.flatten(blk.children.filter(cld => cld._type === portableTextFeatures.types.span.name).map(cld => cld.marks || [])));
...
function isEqualToEmptyEditor(children, portableTextFeatures) {
  return children === void 0 || children && Array.isArray(children) && children.length === 0 || children && Array.isArray(children) && children.length === 1 && slate.Element.isElement(children[0]) && children[0]._type === portableTextFeatures.types.block.name && "style" in children[0] && children[0].style === portableTextFeatures.styles[0].value && Array.isArray(children[0].children) && children[0].children.length === 1 && slate.Text.isText(children[0].children[0]) && children[0].children[0]._type === "span" && children[0].children[0].text === "";
}
...
function createOperationToPatches(portableTextFeatures) {
  ...
  function insertTextPatch(editor, operation, beforeValue) {
    ...
    const textChild = editor.isTextBlock(block) && slate.Text.isText(block.children[operation.path[1]]) && block.children[operation.path[1]];
    if (!textChild) {
    ...
    const prevText = slate.Text.isText(prevChild) ? prevChild.text : "";
  ...
  function removeTextPatch(editor, operation, beforeValue) {
    ...
    const prevText = slate.Text.isText(prevTextChild) && prevTextChild.text;
    const patch = diffMatchPatch(prevText || "", textChild.text, path);
  ...
  function setNodePatch(editor, operation) {
    if (operation.path.length === 1) {
      ...
      const setNode = lodash.omitBy(_objectSpread(_objectSpread({}, editor.children[operation.path[0]]), operation.newProperties), lodash.isUndefined);
      ...
      if (editor.isTextBlock(block)) {
        ...
        if (child) {
          ...
          Object.keys(operation.newProperties).forEach(keyName => {
            const val = lodash.get(operation.newProperties, keyName);
...
function createArrayedPath(point, editor) {
  ...
  const childIndex = block.children.findIndex(child => lodash.isEqual([{
    _key: child._key
  }], childPath));
    ...
    portableTextEditor.setEditable({
      ...
      addAnnotation: (type, value) => {
        ...
        if (selection) {
          ...
          if (slate.Element.isElement(block) && block._type === portableTextFeatures.types.block.name) {
          ...
          if (range) {
            ...
            const nodes = slate.Editor.nodes(editor, {
              ...
              match: node => {
                ...
                return node._type === portableTextFeatures.types.span.name || !editor.isTextBlock(node) && slate.Element.isElement(node);
              }
            });
...
function findTargetIndex(array, pathSegment) {
  ...
  const index = lodash.findIndex(array, pathSegment);
...
function apply$2(value, patch) {
  const nextValue = lodash.clone(value);
  if (patch.path.length === 0) {
    if (patch.type === "set") {
...
function createPatchToOperations(portableTextFeatures, keyGenerator) {
  ...
  function setPatch(editor, patch) {
    ...
    if (isTextBlock) {
      ...
      if (targetPath.length === 1) {
        ...
        const onSamePath = prevSel && lodash.isEqual(prevSel.focus.path, targetPath);
        if (onSamePath) {
  ...
  function unsetPatch(editor, patch, previousPatch2) {
    ...
    if (isTextBlock && patch.path[1] === "children" && patch.path.length === 3) {
      ...
      const prevSel = editor.selection && _objectSpread({}, editor.selection);
      const onSamePath = lodash.isEqual((_a = editor.selection) == null ? void 0 : _a.focus.path, targetPath);
      ...
      if (prevSel && onSamePath && editor.isTextBlock(block)) {
        ...
        if (needToAdjust) {
          const isMergeUnset = (previousPatch2 == null ? void 0 : previousPatch2.type) === "set" && previousPatch2.path[3] === "text" && typeof previousPatch2.value === "string" && lodash.isEqual(previousPatch2.value.slice(-block.children[childIndex].text.length), block.children[childIndex].text);
          if (isMergeUnset) {
      ...
      if (undos.length > 0) {
        ...
        if (step.operations.length > 0) {
          ...
          otherPatches.forEach(item => {
            transformedOperations = lodash.flatten(transformedOperations.map(op => transformOperation(editor, item.patch, op)));
          });
...
function transformOperation(editor, patch, operation) {
  ...
  if (patch.type === "diffMatchPatch") {
    const blockIndex = editor.children.findIndex(blk => lodash.isEqual({
      _key: blk._key
    }, patch.path[0]));
    const block = editor.children[blockIndex];
...
function adjustBlockPath(editor, patch, operation, level) {
  const myIndex = editor.children.findIndex(blk => lodash.isEqual({
    _key: blk._key
  }, patch.path[0]));
...
function createWithPatches(_ref6) {
  ...
  return [function withPatches(editor) {
    ...
    const syncValueAfterIncomingPatches = lodash.debounce(() => syncValue(), 100, {
      trailing: true,
      leading: false
    });
    if (incomingPatches$) {
...
function createWithPortableTextBlockStyle(portableTextFeatures, change$) {
  ...
  return function withPortableTextBlockStyle(editor) {
    ...
    editor.pteToggleBlockStyle = blockStyle => {
      ...
      const selectedBlocks = [...slate.Editor.nodes(editor, {
        at: editor.selection,
        match: node => slate.Element.isElement(node) && node._type === portableTextFeatures.types.block.name
      })];
...
function createWithPortableTextMarkModel(portableTextFeatures, keyGenerator) {
  return function withPortableTextMarkModel(editor) {
    ...
    editor.normalizeNode = nodeEntry => {
      ...
      const isSpan = slate.Text.isText(node) && node._type === portableTextFeatures.types.span.name;
        ...
        for (const op of editor.operations) {
          if (op.type === "merge_node" && op.path.length === 1 && "markDefs" in op.properties && op.properties._type === portableTextFeatures.types.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
            ...
            if (editor.isTextBlock(targetBlock)) {
              ...
              const newMarkDefs = lodash.uniq([...oldDefs, ...op.properties.markDefs]);
              const isNormalized = lodash.isEqual(newMarkDefs, targetBlock.markDefs);
              if (!isNormalized) {
                slate.Transforms.setNodes(editor, {
          ...
          if (op.type === "split_node" && op.path.length === 1 && slate.Element.isElementProps(op.properties) && op.properties._type === portableTextFeatures.types.block.name && "markDefs" in op.properties && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] + 1 < editor.children.length) {
            const [targetBlock, targetPath] = slate.Editor.node(editor, [op.path[0] + 1]);
            debug$9("Copying markDefs over to split block", op);
            if (editor.isTextBlock(targetBlock)) {
              const oldDefs = Array.isArray(targetBlock.markDefs) && targetBlock.markDefs || [];
              slate.Transforms.setNodes(editor, {
                markDefs: lodash.uniq([...oldDefs, ...op.properties.markDefs])
              }, {
                at: targetPath,
                voids: false
              });
          ...
          if (op.type === "split_node" && op.path.length === 1 && op.properties._type === portableTextFeatures.types.block.name && "markDefs" in op.properties && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0) {
            ...
            if (editor.isTextBlock(block) && block.children.length === 1 && block.markDefs.length > 0 && slate.Text.isText(block.children[0]) && block.children[0].text === "" && block.children[0].marks.length === 0) {
              slate.Transforms.setNodes(editor, {
                markDefs: []
              }, {
                at: blockPath
    ...
    editor.addMark = mark => {
      if (editor.selection) {
        if (slate.Range.isExpanded(editor.selection)) {
          ...
          const shouldRemoveMark = lodash.flatten(splitTextNodes.map(item => item[0]).map(node => node.marks)).includes(mark);
          if (shouldRemoveMark) {
    ...
    editor.pteIsMarkActive = mark => {
      ...
      if (slate.Range.isExpanded(editor.selection)) {
        Array.from(slate.Editor.nodes(editor, {
          ...
          const [node] = n;
          existingMarks = lodash.uniq([...existingMarks, ...(node.marks || [])]);
        });
      }
  ...
  return function withInsertData(editor) {
    ...
    editor.insertTextOrHTMLData = data => {
      ...
      if (html || text) {
        ...
        let insertedType;
        if (html) {
          portableText = blockTools.htmlToBlocks(html, portableTextFeatures.types.portableText).map(block => blockTools.normalizeBlock(block, {
            blockTypeName
          }));
          ...
          portableText = blockTools.htmlToBlocks(textToHtml, portableTextFeatures.types.portableText).map(block => blockTools.normalizeBlock(block, {
            blockTypeName
          }));
          fragment = toSlateValue(portableText, {
            portableTextFeatures
    ...
    if (editor.isTextBlock(newNode)) {
      newNode.markDefs = newNode.markDefs.map(def => {
        ...
        if (Array.isArray(newNode.children)) {
          newNode.children = newNode.children.map(child => child._type === spanTypeName && slate.Text.isText(child) ? _objectSpread(_objectSpread({}, child), {}, {
  ...
  constructor(props) {
    ...
    this.syncValue = userCallbackFn => {
      ...
      const isEqualToValue = !(val || []).some((blk, index) => {
        ...
        if (!lodash.isEqual(compareBlock, this.slateInstance.children[index])) {
          return true;
        }
        return false;
      });
    ...
    this.flushDebounced = lodash.debounce(this.flush, FLUSH_PATCHES_DEBOUNCE_MS, {
      leading: false,
      trailing: true
    });
    if (!props.type) {
...
const retrySync = lodash.throttle((syncFn, callbackFn) => syncFn(callbackFn), 100);
...
function compactPatches(patches) {
  ...
  finalPatches = finalPatches.filter((patch, index) => {
    ...
    if (nextPatch && nextPatch.type === "set" && patch.type === "set" && lodash.isEqual(patch.path, nextPatch.path)) {
      return false;
    }
    return true;
  });
...
function normalizeSelection(selection, value) {
  ...
  if (anchor && value.find(blk => lodash.isEqual({
    _key: blk._key
  }, anchor.path[0]))) {
    newAnchor = normalizePoint(anchor, value);
  }
  if (focus && value.find(blk => lodash.isEqual({
    _key: blk._key
  }, focus.path[0]))) {
...
const Leaf = props => {
  ...
  if (slate.Text.isText(leaf) && leaf._type === portableTextFeatures.types.span.name) {
    ...
    const marks = lodash.uniq((Array.isArray(leaf.marks) ? leaf.marks : []).filter(mark => decoratorValues.includes(mark)));
    marks.forEach(mark => {
      var _a, _b;
      ...
      if (type) {
        if ((_a = type == null ? void 0 : type.blockEditor) == null ? void 0 : _a.render) {
          ...
          returnedChildren = /* @__PURE__ */jsxRuntime.jsx(CustomComponent, {
            mark,
    ...
    if (annotations.length > 0) {
      annotations.forEach(annotation => {
        ...
        const defaultRender = () => CustomComponent ? /* @__PURE__ */jsxRuntime.jsx(CustomComponent, _objectSpread(_objectSpread({}, annotation), {}, {
          attributes,
          children: returnedChildren
        })) : /* @__PURE__ */jsxRuntime.jsx(jsxRuntime.Fragment, {
          children: returnedChildren
...
function TextBlock(props) {
  ...
  return /* @__PURE__ */jsxRuntime.jsxs(jsxRuntime.Fragment, {
    children: [!CustomStyle && renderedBlock, CustomStyle && /* @__PURE__ */jsxRuntime.jsx(CustomStyle, {
      style,
      children: renderedBlock
    })]
  });
...
const defaultRender = value => {
  return /* @__PURE__ */jsxRuntime.jsx(DefaultObject, {
    value
  });
};
const Element = _ref20 => {
  ...
  if (element._type === portableTextFeatures.types.block.name) {
    ...
    const textBlock = /* @__PURE__ */jsxRuntime.jsx(TextBlock, {
      block: element,
      portableTextFeatures,
      children
    });
...
const PortableTextEditable = React.forwardRef(function PortableTextEditable2(props, forwardedRef) {
  ...
  const withInsertData = React.useMemo(() => createWithInsertData(change$, portableTextFeatures, keyGenerator), [change$, keyGenerator, portableTextFeatures]);
  const withHotKeys = React.useMemo(() => createWithHotkeys(portableTextFeatures, keyGenerator, portableTextEditor, hotkeys), [hotkeys, keyGenerator, portableTextEditor, portableTextFeatures]);
  React.useMemo(() => {
    if (readOnly) {
